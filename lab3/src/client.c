...
bool parse_and_validate_args(int argc, char *argv[], char **host, int *port, char **name) {
...
char* parse_client_input(const char *input, int *attempts) {
...
void display_game_state(const char *username, int attempts, int min, int max, const char *last_input, const char *last_response) {
...
void game_loop(int sock, int attempts, int min, int max, char *name[]) {
    ...
    while (true) {
        ...
        input[strcspn(input, "\n")] = 0;
        char* parsed_input = parse_client_input(input, &attempts);
